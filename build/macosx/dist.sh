...
then
  ...
  INFO_FOUND=`cat ./dist/Wiring.app/Contents/Info.plist | grep $INFO_SOUGHT`
  if [ -z "$INFO_FOUND" ]
  then 
...
then
  ...
  echo $1 > work/Wiring.app/Contents/Resources/Java/lib/version.txt
fi
...
SOURCE_DIR="work"
SOURCE_FILES="Wiring.app"
OUTPUT_DMG="wiring-$RELEASE"
