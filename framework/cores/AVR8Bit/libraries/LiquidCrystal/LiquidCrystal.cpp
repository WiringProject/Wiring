...
// function overloading lets the list of arguments specify if 4 or 8 data
// lines are used and if rw is used or tied to gnd.
// I can't eliminate rw and add en2 and be unique.
...
// 4x40  4 data pins 2 enable pins
...
// 4x40  4 data pins 2 enable pins
LiquidCrystal::LiquidCrystal(uint8_t rs, uint8_t rw, uint8_t enable, uint8_t en2,
                             uint8_t d0, uint8_t d1, uint8_t d2, uint8_t d3, void userBusy(int8_t chip))
{
...
LiquidCrystal::LiquidCrystal(uint8_t rs, uint8_t enable, uint8_t d0, uint8_t d1, uint8_t d2, uint8_t d3, void userBusy(int8_t chip))
{
  init(rs, 255, enable, 255, d0, d1, d2, d3,  userBusy);
}


void LiquidCrystal::init(uint8_t rs, uint8_t rw, uint8_t enable, uint8_t en2,
                         uint8_t d0, uint8_t d1, uint8_t d2, uint8_t d3,
                         void userBusy(int8_t chip))
{
  _rs_pin = rs;
  _rw_pin = 255;
  ...
  begin(20, 1);
  userFunc = userBusy;  //pointer to user written busy test
...
{
  // there is an implied lack of trust;
  // the private version can't be munged up by the user.
  numcols = _numcols = cols;
  numlines = _numlines = lines;
  // should autoadjust for 16/20 or whatever columns now
  row_offsets[2] = cols + row_offsets[0];
  ...
  {
    // each line gets its own little 40 char section of DDRAM--would be
    // fine if there were a 4x32, I suppose
    row_offsets[2] = 0;
    row_offsets[3] = 0x40;
    _chip = 2;
...
void LiquidCrystal::begin2(uint8_t cols, uint8_t lines, uint8_t dotsize, uint8_t enable)
{
  ...
  // before sending commands. Atmel AVRs can turn on way before 4.5V so we'll
  // wait 50
  for (uint8_t i = 0; i < 18; i++)
  {
    delayMicroseconds(7500);
    // I don't think I can adequately test this number; it will depend a little
    // on which Atmel AVR you have and probably could also vary with the power
    // source applied to that board. The way to test is really to load your
    // program, remove power and then reapply power so that the program starts
    // up as power is applied. If anyone finds they need a larger number please
    // let me know: raine001 at tc dot umn dot edu.
    // delayMicroseconds takes an unsigned int so 65535 is the max.
  }
  ...
  // at this point we are in 8 bit mode but of course in this interface 4 pins
  // are dangling unconnected and the values on them don't matter for these
  // instructions.
  digitalWrite(_rs_pin, LOW);
  write4bits(0x03);
...
// To count the number of times we scrolled; here we'd need to keep track
// of microseconds and divide. I'm not going there.
...
void LiquidCrystal::commandBoth(uint8_t value)
{
  if (_en2 == 255)
  {
...
{
  // first we call setCursor and send the character
  if ((_scroll_count != 0) || (_setCursFlag != 0)) setCursor(_x, _y);

  if ((value != '\r') && (value != '\n')) send(value, HIGH);

  _setCursFlag = 0;
  // then we update the x & y location for the NEXT character
  if (_direction == LCD_Right)
  {
    _x++;
    //romance languages go left to right
    if ((value == '\r') || (_x >= _numcols))
    {

      _x = 0;
      _y++;
      // we'll need a setCursor() before the next char to move to begin of next line
      _setCursFlag = 1;
    }
  }
  else
  {
    _x--;
    // emulate right to left text mode which is built in but would be defeated
    // by my code above
    if ((value == '\n') || (_x < 0))
    {
      _x = _numcols - 1;
  ...
  //wrap last line up to line 0
  if (_y >= _numlines) _y = 0;
}
...
void LiquidCrystal::send(uint8_t value, uint8_t mode)
{
  ...
  {
    {
      if (userFunc != NULL) userFunc(_chip);
      else delayMicroseconds(DELAYPERCHAR);
    }
  }
  else
  {
...
void LiquidCrystal::write4bits(uint8_t value)    // still used during init
{
  ...
  digitalWrite(*pinptr++, (v >>= 1) & 01);
  byte en = _enable_pin;
  // 4x40 LCD with 2 controller chips with separate enable lines if we called
  // w 2 enable pins and are on lines 2 or 3 enable chip 2
  if ((_en2 != 255) && (_chip)) en = _en2;
  digitalWrite(en, HIGH);   // enable pulse must be >450ns
  digitalWrite(en, LOW);
}
