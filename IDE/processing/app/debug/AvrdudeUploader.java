...
public class AvrdudeUploader extends Uploader  {
  ...
  public boolean avrdude(Collection params) throws RunnerException {
    ...
    if (Base.isLinux()) {
      ...
      //commandDownloader.add("-C" + Base.getHardwarePath() + "/tools/avr/bin/avrdude.conf");
      commandDownloader.add("-C" + Base.getAvrBasePath() + "avrdude.conf");
    } else {
      //commandDownloader.add("-C" + Base.getHardwarePath() + "/tools/avr/bin/avrdude.conf");
      commandDownloader.add("-C" + Base.getAvrBasePath() + "avrdude.conf");
    }
