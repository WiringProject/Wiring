...
public class Serial implements SerialPortEventListener {
  ...
  public Serial(String iname) throws SerialException {
    this(iname, Preferences.getInteger("serial.rate"),
         Preferences.get("serial.parity").charAt(0),
         Preferences.getInteger("serial.databits"),
         new Float(Preferences.get("serial.stopbits")).floatValue());
  }
  ...
  public Serial(String iname, int irate,
                 char iparity, int idatabits, float istopbits) throws SerialException {
    ...
    try {
      ...
      throw new SerialException("Serial port " + iname + " already in use. Check no other program is using it.", e);
    } catch (Exception e) {
      port = null;
      input = null;
      output = null;     
    ...
    if (port == null) {
      //System.err.println("Serial port '" + iname + "' not found.  Did you select the right one from the Tools > Serial Port menu?");
  ...
  public void reset() {
    try {
      if (port != null) port.setDTR(true);  // close the port
    } catch (Exception e) {
      e.printStackTrace();
    }
