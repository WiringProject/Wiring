...
public class Preferences {
  ...
  JCheckBox monitorStartBox;
  JCheckBox playBeepBox;
  ...
  static public void init(String commandLinePrefs) {
    ...
    try {
      ...
      Base.showError(null, "Could not read default settings.\n" +
                           "You'll need to reinstall Wiring.", e);
    }
  ...
  public Preferences() {
    ...
    label = new JLabel("  (requires restart of Wiring)");
    box.add(label);
    pain.add(box);
...
*/
    ...
    deletePreviousBox =
      new JCheckBox("Delete previous exported code folder on upload");
    ...
    // [ ] Automatic start of serial monitor after upload
    
    monitorStartBox = new JCheckBox("Automatic start for serial monitor after upload");
    pain.add(monitorStartBox);
    d = monitorStartBox.getPreferredSize();
    monitorStartBox.setBounds(left, top, d.width + 10, d.height);
    right = Math.max(right, left + d.width);
    top += d.height + GUI_BETWEEN;
    

    // [ ] play a Beep after upload or compile
    
    playBeepBox = new JCheckBox("Play a beep after upload or compile");
    pain.add(playBeepBox);
    d = playBeepBox.getPreferredSize();
    playBeepBox.setBounds(left, top, d.width + 10, d.height);
    right = Math.max(right, left + d.width);
    top += d.height + GUI_BETWEEN;
    ...
    label = new JLabel("Select reference language: ");
    pain.add(label);
    d = label.getPreferredSize();
    label.setForeground(Color.gray);
    label.setBounds(left, top, d.width, d.height);
    right = Math.max(right, left + d.width);
    referenceLanguage = new JComboBox();
    
    String availableTranslations = get("reference.translations");
    String translations[];
    translations = PApplet.split(availableTranslations, ',');
    for (int i = 0; i < translations.length; i++)
      referenceLanguage.addItem(translations[i]);
    
    referenceLanguage.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent event) {
        String wholeString = (String) referenceLanguage.getSelectedItem();
        set("reference.language", wholeString);
      }});
    pain.add(referenceLanguage);
    d2 = referenceLanguage.getPreferredSize();
    referenceLanguage.setBounds(left + d.width + 10, top, d2.width + 10, d2.height);
    right = Math.max(right, left  + d.width + 10 + d2.width);
    top += d2.height + GUI_BETWEEN;
    
    // [ ] Automatically associate .pde files with Processing

    if (Base.isWindows()) {
      autoAssociateBox =
        new JCheckBox("Automatically associate .pde files with Wiring");
    ...
    label = new JLabel("(edit only when Wiring is not running)");
    pain.add(label);
    d = label.getPreferredSize();
    label.setForeground(Color.gray);
    label.setBounds(left, top, d.width, d.height);
...
//               closingLastQuitsBox.isSelected());
    ...
    setBoolean("monitor.start", monitorStartBox.isSelected()); 
    setBoolean("editor.beep.compile",playBeepBox.isSelected());
    set("reference.language", (String) referenceLanguage.getSelectedItem());
...
//      setSelected(getBoolean("export.applet.separate_jar_files"));
    ...
    monitorStartBox.
      setSelected(getBoolean("monitor.start"));
    playBeepBox.
      setSelected(getBoolean("editor.beep.compile"));
    
    referenceLanguage.setSelectedItem(get("reference.language"));
